...
//  delay(1000);
  ...
  if (Serial.available()) {
    delay(2);
    bleuart.println(Serial.readString());
...
{
  Serial.println("prph_connect_callback");
  char peer_name[32] = { 0 };
...
{
  Serial.println("prph_disconnect_callback");
  (void) conn_handle;
  (void) reason;
...
{
  Serial.println("prph_bleuart_rx_callback");
...
{
  Serial.println("cent_connect_callback");
  char peer_name[32] = { 0 };
  Bluefruit.Gap.getPeerName(conn_handle, peer_name, sizeof(peer_name));
...
{
  Serial.println("cent_disconnect_callback");
  (void) conn_handle;
  (void) reason;
